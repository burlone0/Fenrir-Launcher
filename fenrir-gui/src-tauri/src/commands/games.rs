...
pub async fn configure_game(
    ...
    let (game, runtime, prefix_dir, profiles_dir) = {
        ...
        let runtime = runtimes
            ...
            .find(|r| {
                game.runtime_id
                    .as_ref()
                    .map(|rid| rid == &r.id.to_string())
                    .unwrap_or(false)
                    || r.is_default
            })
            .ok_or_else(|| "no runtime available — install a runtime first".to_string())?;

        let prefix_dir = dirs::data_dir()
            .map(|d| d.join("fenrir/prefixes"))
            .unwrap_or_default();
    ...
    let wine_bin = runtime.path.join("files/bin/wine");
    let wine_bin = if wine_bin.exists() {
        wine_bin
    } else {
        runtime.path.clone()
    };
    ...
    let _ = clean; // clean flag reserved for future cleanup integration
